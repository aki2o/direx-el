...
(defun direx:item-collapse-recursively (item &optional collapse-myself)
  (when collapse-myself
    (direx:item-ensure-closed item))
  (dolist (child (direx:item-children item))
    (direx:item-collapse-recursively child t)))
...
(defun direx:goto-item-for-tree-1 (tree)
  (cl-loop with children = (direx:item-children direx:root-item)
           with idx = -1
...
(defvar direx:direx-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-S-e")       'direx:collapse-item-recursively)
